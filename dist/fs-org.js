...
shell
    ...
    if (!file.isDirectory()) {
        shell.mkdir('-p', path.join(destDir, moment_1.default(file.mtimeMs).format('YYYY-MM-DD')));
        let currentlocation = file.path;
        let newlocation = path.join(destDir, moment_1.default(file.mtimeMs).format('YYYY-MM-DD'), file.name);
        if (shell.test('-f', newlocation)) {
            console.log('Exists');
        }
        else {
            console.log(colors.green('moving: '), colors.yellow(currentlocation), colors.grey('==>'), colors.yellow(newlocation));
...
//   });
// }
//# sourceMappingURL=fs-org.js.map